...
#include "snmp_lib_default_store.h"     /* for "internal" definitions */
...
ds_toggle_boolean(int storeid, int which)
{
    ...
    DEBUGMSGTL(("ds_toggle_boolean", "Setting %d:%d = %d/%s\n", storeid,
                which, ds_booleans[storeid][which / 8],
...
ds_get_boolean(int storeid, int which)
{
    ...
    return ((ds_booleans[storeid][which / 8] & (1 << (which % 8))) ? 1 :
            0);
}
    ...
    if (drsp != NULL) {
        ...
        case ASN_INTEGER:
            ...
            if (*endptr != 0)
                ...
                ds_set_int(drsp->storeid, drsp->which, itmp);
                   ...
                   int storeid, int which)
{
    ...
    switch (type) {
    case ASN_BOOLEAN:
        register_config_handler(ftype, token, ds_handle_config, NULL,
                   ...
                   int storeid, int which)
{
    ...
    switch (type) {
    case ASN_BOOLEAN:
        register_prenetsnmp_mib_handler(ftype, token, ds_handle_config,
