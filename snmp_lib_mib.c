                         ...
                         int allow_realloc, const u_char * cp, size_t len)
{
    ...
    if ((lenleft > 0)
        && ds_get_boolean(DS_LIBRARY_ID, DS_LIB_PRINT_HEX_TEXT)) {
...
int
#ifdef __vms
sprint_realloc_object_id(u_char ** buf, size_t * buf_len,
                                 size_t * out_len, int allow_realloc,
                                 netsnmp_variable_list * var,
                                 struct enum_list *enums,
                                 const char *hint, const char *units)
...
#ifdef __vms
    netsnmp_sprint_realloc_objid_tr(buf, buf_len, out_len, allow_realloc,
                                      &buf_overflow,
                                      (oid *) (var->val.objid),
...
#ifdef __vms
    return snmp_strcat(buf, buf_len, out_len, allow_realloc, (unsigned char*) tmp);
#else
                       ...
                       const char *hint, const char *units)
{
    ...
    case ASN_OBJECT_ID:
#ifdef __vms
        return sprint_realloc_object_id(buf, buf_len, out_len,
...
set_function(struct tree *subtree)
{
    ...
    case TYPE_OBJID:
#ifdef __vms
        subtree->printomat = sprint_realloc_object_id;
#else
...
struct tree    *
#ifdef __vms
netsnmp_sprint_realloc_objid_tr(u_char ** buf, size_t * buf_len,
                                  size_t * out_len, int allow_realloc,
                                  int *buf_overflow,
...
#endif
{
    ...
    if (ds_get_boolean(DS_LIBRARY_ID, DS_LIB_PRINT_NUMERIC_OIDS)) {
        ...
        if (ds_get_int(DS_LIBRARY_ID, DS_LIB_PRINT_SUFFIX_ONLY) == 2
            && cp > tbuf) {
...
#ifdef __vms
        netsnmp_sprint_realloc_objid_tr(&buf, &buf_len, &out_len, 1,
                                          &buf_overflow, objid, objidlen);
#else
                        ...
                        netsnmp_variable_list * variable)
{
    ...
    subtree =
#ifdef __vms
        netsnmp_sprint_realloc_objid_tr(buf, buf_len, out_len,
                    ...
                    struct index_list *in_dices, size_t * end_of_known)
{
    ...
    int             extended_index =
        ds_get_boolean(DS_LIBRARY_ID, DS_LIB_EXTENDED_INDEX);
    char            intbuf[64];
        ...
        case TYPE_OBJID:
            ...
            if (extended_index) {
                if (in_dices->isimplied) {
                    if (!*buf_overflow
#ifdef __vms
                        && !netsnmp_sprint_realloc_objid_tr(buf, buf_len,
                                                              out_len,
                                                              allow_realloc,
                                                              buf_overflow,
                                                              objid,
...
void
#ifdef __vms
print_oid_report_en_labeledoid(void)
#else
...
void
#ifdef __vms
print_oid_report_en_symbolic(void)
#else
