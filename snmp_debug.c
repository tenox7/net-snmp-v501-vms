...
#include "snmp_debug.h"	           /* For this file's "internal" definitions */
    ...
    if (var_subid == 0) {
        ...
        if (rc) {
            sprintf(tmpbuf, ".%lu--%lu", theoid[var_subid - 1],
                    range_ubound);
#ifdef __vms
            rc = snmp_strcat(&buf, &buf_len, &out_len, 1, (unsigned char*) tmpbuf);
#else
...
#ifdef __vms
                    if (!snmp_strcat(&buf, &buf_len, &out_len, 1, (unsigned char*) tmpbuf)) {
#else
