...
_asn_length_err(const char *str, size_t wrongsize, size_t rightsize)
{
    ...
    sprintf(ebuf, "%s length %d too large: exceeds %d", str, wrongsize,
            rightsize);
    ERROR_MSG(ebuf);
}
                        ...
                        size_t datalen, size_t typedlen)
{
    ...
    if (datalen < typedlen) {
        sprintf(ebuf, "%s: bad header, length too short: %d < %d", str,
                datalen, typedlen);
        ERROR_MSG(ebuf);
        return 1;
    }
                 ...
                 size_t * datalength, u_char type, size_t length)
{
    ...
    if (*datalength < 1) {
        sprintf(ebuf, "bad header length < 1 :%d, %d", *datalength,
                length);
        ERROR_MSG(ebuf);
        return NULL;
    }
                   ...
                   size_t * datalength, u_char type, size_t length)
{
    ...
    if (*datalength < 4) {
        sprintf(ebuf, "%s: length %d < 4: PUNT", errpre,
                (int) *datalength);
        ERROR_MSG(ebuf);
        return NULL;
    }
...
asn_build_length(u_char * data, size_t * datalength, size_t length)
{
    ...
    if (length < 0x80) {
        if (*datalength < 1) {
            sprintf(ebuf, "%s: bad length < 1 :%d, %d", errpre,
                    *datalength, length);
            ERROR_MSG(ebuf);
            return NULL;
        }
        *data++ = (u_char) length;
        ...
        if (*datalength < 2) {
            sprintf(ebuf, "%s: bad length < 2 :%d, %d", errpre,
                    *datalength, length);
            ERROR_MSG(ebuf);
            return NULL;
        }
        ...
        *data++ = (u_char) length;
        ...
        if (*datalength < 3) {
            sprintf(ebuf, "%s: bad length < 3 :%d, %d", errpre,
                    *datalength, length);
            ERROR_MSG(ebuf);
            return NULL;
        }
                          ...
                          u_char type, size_t length)
{
    ...
    if (asn_realloc_rbuild_length(pkt, pkt_len, offset, r, length)) {
        if (((*pkt_len - *offset) < 1)
            && !(r && asn_realloc(pkt, pkt_len))) {
            sprintf(ebuf, "bad header length < 1 :%d, %d",
                    *pkt_len - *offset, length);
            ERROR_MSG(ebuf);
            return 0;
        }
...
int
#ifdef __vms
asn_realloc_rbuild_u_int64(u_char ** pkt, size_t * pkt_len,
                                  size_t * offset, int r,
                                  u_char type,
                                  struct counter64 *cp, size_t countersize)
#else
