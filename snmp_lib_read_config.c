...
#include "snmp_lib_read_config.h"       /* for "internal" definitions */
...
struct config_line *
#ifdef __vms
register_app_prenetsnmp_mib_hnd(const char *token,
                                    void (*parser) (const char *, char *),
                                    void (*releaser) (void),
                                    const char *help)
#else
...
void
#ifdef __vms
netsnmp_config_rem_free_list(struct read_config_memory **mem)
#else
...
void
#ifdef __vms
netsnmp_config_proc_mem_list(struct read_config_memory **memp,
                                   int when, int clear)
#else
...
#endif
{
    ...
    if (clear)
#ifdef __vms
        netsnmp_config_rem_free_list(memp);
#else
...
{
#ifdef __vms
    netsnmp_config_proc_mem_list(&memorylist, EITHER_CONFIG, 1);
#else
...
void
#ifdef __vms
netsnmp_config_proc_mem_when(int when, int clear)
#else
netsnmp_config_process_memories_when(int when, int clear)
#endif
{
#ifdef __vms
    netsnmp_config_proc_mem_list(&memorylist, when, clear);
#else
...
read_configs(void)
{
    ...
    netsnmp_config_proc_mem_when(NORMAL_CONFIG, 1);
...
read_premib_configs(void)
{
    ...
    netsnmp_config_proc_mem_when(PREMIB_CONFIG, 0);
...
#endif
#ifdef SNMPSHAREPATH
                ENV_SEPARATOR_CHAR, SNMPSHAREPATH, 
#else
		'\0', "",
#endif
#ifdef SNMPLIBPATH
