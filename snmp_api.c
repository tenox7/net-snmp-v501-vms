...
#include "snmp_lib_snmp.h"      /* for xdump & {build,parse}_var_op */
#include "snmp_api.h"
...
snmp_api_errstring(int snmp_errnumber)
{
    ...
    if (snmp_detail_f) {
#ifdef __vms
        snprintf(msg_buf, (unsigned int) 256, "%s (%s)", msg, snmp_detail);
#else
...
snmp_open_ex(netsnmp_session * session,
             ...
             int (*fparse) (netsnmp_session *, netsnmp_pdu *, u_char *,
                            size_t), int (*fpost_parse) (netsnmp_session *,
                                                         netsnmp_pdu *,
                                                         int),
...
#ifdef __vms
    size_t          length = 0, pdulen = 0, rxbuf_len = 8192;
#else
...
#ifdef __vms
/*
 * like snmp  but does not calculate the timeout or blocking
 * parameters. This is suitable for use within the UTC where the UTC s/w
 * sets a timeout < 1 second and calls timeout() periodically.
 * The UTC SNMP spends a lot of time in here so I've removed the unwanted bits
 * to make it faster.
 */
int
snmp_select_info_2 (int *numfds,
			fd_set * fdset)
{
    struct session_list *slp;
    int			active	= 0;
...
snmp_pdu_add_variable(netsnmp_pdu *pdu,
                      oid * name,
                      size_t name_length,
                      u_char type, u_char * value, size_t len)
{
