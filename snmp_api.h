...
#define USM_PRIV_KU_LEN     32
    ...
    struct snmp_session {
         ...
         */
        u_char         *(*authenticator) (u_char *, size_t *, u_char *,
                                          size_t);
...
#ifdef __vms
    /*
     * This is the same as snmp_select_info() except that the timeout and
     * blocking parameters are not setup. This works on UTC where select()
     * timeouts are a lot less than 1 sec and timeout is called() periodically.
     */

    int             snmp_select_info_2(int *, fd_set *);
#endif
...
#define NETSNMP_CALLBACK_OP_DISCONNECT		5
    ...
    int
        ...
        snmp_pdu_realloc_rbuild(u_char ** pkt, size_t * pkt_len,
